    ...
    public static boolean intersect(Segment s1, Segment s2) {
        if (!s1.isVertical() && !s2.isVertical()) {
            if (!Rational.equals(s1.getA(), s2.getA())) {
                Rational x = Rational.dividedBy(Rational.minus(s2.getB(), s1.getB()),
                        Rational.minus(s1.getA(), s2.getA()));
                return (Rational.lesserEqual(s1.getStartX(), x) && Rational.lesserEqual(s2.getStartX(), x)
                        && Rational.lesserEqual(x, s1.getEndX()) && Rational.lesserEqual(x, s2.getEndX()));
            } else {
                if (Rational.equals(s1.getB(), s2.getB())) {
                    return ((Rational.lesserEqual(s1.getStartX(), s2.getStartX())
                            && Rational.lesserEqual(s2.getStartX(), s1.getEndX()))
                            || (Rational.lesserEqual(s2.getStartX(), s1.getStartX())
                                    && Rational.lesserEqual(s1.getStartX(), s2.getEndX())));
                } else {
                    return false;
                }
            }
        ...
        if (!s2.isVertical()) {
            ...
            return (Rational.lesserEqual(s2.getStartX(), s1.getStartX())
                    && Rational.lesserEqual(s1.getStartX(), s2.getEndX()) && Rational.lesserEqual(s1.getStartY(), y)
                    && Rational.lesserEqual(y, s1.getEndY()));
        } else {
